        ...
        <div class="bg-white border border-secondary/30 rounded-lg p-6">
            ...
            <form method="GET" action="{{ route('user.campaign') }}" class="mb-4">
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex-1">
                        <label class="text-xs font-medium text-gray-700 mb-1 block">Filter by Member</label>
                        <select name="campaign_{{ $campaign->id }}_member" onchange="this.form.submit()" class="w-full px-3 py-2 text-sm border border-secondary/30 rounded-lg focus:border-primary focus:ring-1 focus:ring-primary/20">
                            <option value="">All Members</option>
                            @foreach($campaign->members as $member)
                            <option value="{{ $member->pivot->id }}" {{ ($campaign->filterMember ?? '') == $member->pivot->id ? 'selected' : '' }}>{{ $member->name }}</option>
                            @endforeach
                        </select>
                    </div>
